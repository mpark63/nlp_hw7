...
class Kialo:
    ...
    def closest_claims(self, s: str, n: int = 1, kind: str = 'all', threshold = None) -> List['Kialo.Claim']:
        ...
        if self.claims[kind]:
            if threshold is None:
                log.info(f"BM25 query: {s}")
                return self.bm25[kind].get_top_n(self.tokenizer(s), self.claims[kind], n=n)
            else: 
                top_n = []
                scores = self.bm25[kind].get_scores(self.tokenizer(s))
                top_n_scores = np.sort(scores)[::-1][:n]
                top_n_index = np.argsort(scores)[::-1][:n]

                for i in range(len(top_n_scores)):
                    if top_n_scores[i] >= threshold:
                        top_n.append(top_n_index[i])
                return [self.claims[kind][i] for i in top_n] 
